defmodule Nadia.GraphTest do
  ...
  setup_all do
    Config.filter_sensitive_data("bot[^/]+/", "bot<TOKEN>/")
    Config.filter_sensitive_data("id\":\\d+", "id\":666")
    Config.filter_sensitive_data("id=\\d+", "id=666")
    Config.filter_sensitive_data("_id=@w+", "_id=@group")
    :ok
  end

  test "create_account" do
    use_cassette "graph/create_account" do
      {:ok, account} = Graph.create_account("some_short_name", "some_author_name")
  ...
  test "revoke_access_token" do
    use_cassette "graph/revoke_access_token" do
      {:ok, account} =
        Graph.revoke_access_token("b968da509bb76866c35425099bc0989a5ec3b32997d55286c657e6994bbb")
  ...
  test "get_page" do
    use_cassette "graph/get_page" do
      {:ok, page} = Graph.get_page("Sample-Page-12-15")
      assert page.title == "Sample Page"
  ...
  test "get_views" do
    use_cassette "graph/get_views" do
      {:ok, result} = Graph.get_views("Sample-Page-12-15", year: 2012, month: 12)
